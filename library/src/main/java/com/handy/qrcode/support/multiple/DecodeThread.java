...
public final class DecodeThread extends Thread {
    ...
    DecodeThread(ScanMultipleActivity activity, Collection<BarcodeFormat> decodeFormats, Map<DecodeHintType, ?> baseHints, String characterSet, ResultPointCallback resultPointCallback) {
        ...
        if (decodeFormats == null || decodeFormats.isEmpty()) {
            ...
            if (ScanConfig.KEY_DECODE_1D_PRODUCT) {
                decodeFormats.addAll(DecodeFormatManager.PRODUCT_FORMATS);
            }
            if (ScanConfig.KEY_DECODE_1D_INDUSTRIAL) {
                decodeFormats.addAll(DecodeFormatManager.INDUSTRIAL_FORMATS);
            }
            if (ScanConfig.KEY_DECODE_QR) {
                decodeFormats.addAll(DecodeFormatManager.QR_CODE_FORMATS);
            }
            if (ScanConfig.KEY_DECODE_DATA_MATRIX) {
                decodeFormats.addAll(DecodeFormatManager.DATA_MATRIX_FORMATS);
            }
            if (ScanConfig.KEY_DECODE_AZTEC) {
                decodeFormats.addAll(DecodeFormatManager.AZTEC_FORMATS);
            }
            if (ScanConfig.KEY_DECODE_PDF417) {
